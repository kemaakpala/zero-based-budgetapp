...
- [ ] **Linked Budget Items**: Allow a Budget Item in one group (e.g. "Rent / Mortgage" in Housing) to be linked to a Debt Item in the Debt Repayment Group, so that payment transactions recorded against either item update the debt's Outstanding Balance. This would enable tracking mortgage payoff progress without requiring the user to move the item out of its natural Budget Group and avoids double-assignment of salary. Emerged from the debt tracking design session as Approach 3 for the mortgage use case.
- [ ] **Copy from Previous Month Initialisation**: As an alternative to initialising from the Budget Template, offer users the option to bootstrap a new month by copying the previous month's Budget Groups, Budget Items, and assigned amounts (but not Transactions). For Debt Items, this would also carry forward the current Outstanding Balance. This is a UX convenience feature — the monthly data blob remains independent after creation (no permanent cross-month dependency).
