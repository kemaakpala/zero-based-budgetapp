...
- [ ] **Linked Budget Items**: Allow a Budget Item in one group (e.g. "Rent / Mortgage" in Housing) to be linked to a Debt Item in the Debt Group, so that payment transactions recorded against either item update the debt's Outstanding Balance. This would enable tracking mortgage payoff progress without requiring the user to move the item out of its natural Budget Group and avoids double-assignment of salary. Emerged from the debt tracking design session as Approach 3 for the mortgage use case.
