...
import type { DebtFormData } from "../../Component/DebtGroup/DebtFormModal";
import type { SavingsFormData } from "../../Component/SavingsGroup/SavingsFormModal";
...
function Budget() {
  ...
  const cycleCalculator = useMemo(() => {
    return new BudgetCycleCalculator({
      ...
      weekendBehavior: state.weekendBehavior as
        | "preceding-friday"
        | "following-monday"
        | "exact",
    });
  ...
  const [activeAddTransactionItem, setActiveAddTransactionItem] =
    useState<EnrichedBudgetItem | null>(null);
  ...
  const [editingDebtItem, setEditingDebtItem] = useState<BudgetItem | null>(
    null
  );

  // Savings Modal States
  const [savingsFormModalOpen, setSavingsFormModalOpen] = useState(false);
  const [editingSavingsItem, setEditingSavingsItem] =
  ...
  const handleRenameGroup = (groupIndex?: number, currentName?: string) => {
    if (groupIndex === undefined || currentName === undefined) return;
  ...
  const handleDeleteGroup = (groupIndex?: number, groupName?: string) => {
    if (groupIndex === undefined || groupName === undefined) return;
  ...
  const handleAddDebtItem = (debtData: DebtFormData) => {
    hookAddDebtItem(debtData);
    setDebtFormModalOpen(false);
  };

  const handleUpdateDebtItem = (debtData: DebtFormData) => {
    if (!debtData.itemId) return;
    hookUpdateDebtItem(debtData as DebtFormData & { itemId: string });
    setEditingDebtItem(null);
    setDebtFormModalOpen(false);
  };

  const handleAddSavingsItem = (savingsData: SavingsFormData) => {
    hookAddSavingsItem(savingsData);
    setSavingsFormModalOpen(false);
  };

  const handleUpdateSavingsItem = (savingsData: SavingsFormData) => {
    if (!savingsData.itemId) return;
    hookUpdateSavingsItem(savingsData as SavingsFormData & { itemId: string });
  ...
  return (
    <section key={monthKey}>
      ...
      <BudgetForm onAddGroupClick={handleAddGroup}>
        {enrichedBudgetGroups.map((group, groupIndex) => {
          const budgetGroupElement = group.isDebtGroup ? (
            ...
            <BudgetGroup
              ...
              viewMode={viewMode}
              onSaveField={handleFieldChange}
              onAddTransactionClick={(_gIdx, _iIdx, item) => {
                if (item) setActiveAddTransactionItem(item);
              }}
              onViewTransactionsClick={(_gIdx, _iIdx, item) => {
                if (item) setActiveViewTransactionsItem(item);
              }}
              onDeleteItemClick={handleDeleteItem}
